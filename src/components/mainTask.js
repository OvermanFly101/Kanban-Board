...
let backendItems = []; 
//{id: uuid(), content: itemList}
//{id: uuid(), content: itemList}

const backend = (itemList) => {
  itemList.map((item) => backendItems.push({id:uuid(), 'content': item}))
...
const onDragEnd = (result, columns, setColumns) => {
  if(!result.destination) {
    console.log("You must drop inside the column. This is not a droppable area.");
    return;
  }
  const { source, destination } = result;
  if(SourceBuffer.droppableId !== destination.droppableId) {
